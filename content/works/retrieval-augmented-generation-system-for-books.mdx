---
title: Retrieval Augmented Generation System for Books
description: Developing a local RAG system using Llama Index and Llama 3.1
date: 2024-05-30
tag: RAG
image: /assets/works/retrieval-augmented-generation-system-for-books/thumbnail.png
type: Work
---

**Tech stack**: [Gradio](https://www.gradio.app/), [Hugging Face](https://huggingface.co/BAAI/bge-large-en-v1.5), [Llama 3.1](https://llama.meta.com/), [Llama Index](https://www.llamaindex.ai/), [Ollama](https://ollama.com/), Python, and [Weaviate](https://weaviate.io/)

**Update**:
1. Developed a user interface.
...
**_TL;DR_**: This is a local RAG system to retrieve information and summaries with or without books' metadata.
In addition to that, users have control on how the search can be done through a UI. Even though this system is capable of providing relatively accurate responses, this RAG system is still generating summaries in real time. Meaning that the system has to go through thousand of chunks of documents whenever users ask for book summaries, and it's considered computationally expensive.

## Introduction

This project is a local Retrival Augmented Generation (RAG) system that retrieves and summarizes books.
Since I have over 200 books in my library, I often find it difficult to discover interesting readings.

I created this project to help me summarize books that I have never read before. If the summary is interesting,
I would consider spend time reading the book. If not, I would love to just read the summary of each chapter of the book.

![What Llama Index and Langchain focus on (Source: https://superwise.ai/blog/lets-talk-about-llamaindex-and-langchain/)](/assets/works/retrieval-augmented-generation-system-for-books/langchain-llama-index.png)
...
![1024 chunk size, and 100 chunk overlap.](/assets/works/personalized-retrieval-augmented-generation-system/chunk-visualization.png)
