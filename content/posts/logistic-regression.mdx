...
Why do we need a Sigmoid function? Look at the following figure:
![Non-linearly separable data](/assets/posts/logistic-regression/non-linear.png)

The figure above shows that the data is not linearly separable.
Thus, we need the help of the Sigmoid function to make the data separable.
...
Note that $-1.2$ is a random value for the intercept and $0.28$ is a random value for the coefficient.
...
Once we have the necessary functions, we can start training the model.
...
Let's plot the graph again with the Logistic Regression line.
