...
Similar to what we did in [the Linear Regression post](/posts/linear-regression), we need to estimate the best $\beta_0$ and $\beta_1$ using the Gradient Descent algorithm.
...
\cdots \\
\beta_p = \beta_p - \alpha \frac{\partial}{\partial \beta_n} J(\beta_0, \beta_1, \dots, \beta_p)x_p \\
\end{gather*}
$$
