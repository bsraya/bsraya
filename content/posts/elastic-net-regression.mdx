...
$$
\min _\beta\left( \frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2 + \lambda_1 \sum_{j=1}^{p} |\beta_j| + \lambda_2 \sum_{j=1}^{p} |\beta_j^2| \right)
$$

where
1. $\frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2$ is Mean Squared Error.
2. $\lambda_1 \sum_{j=1}^{p} |\beta_j|$ is Lasso Penalty, where $\beta_j$ is the coefficient of the $j^{th}$ feature.
3. $\lambda_2 \sum_{j=1}^{p} |\beta_j^2|$ is Ridge Penalty, where $\beta_j^2$ is the square of the coefficient of the $j^{th}$ feature.
