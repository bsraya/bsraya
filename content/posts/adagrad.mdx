...
Most implementations leave the learning rate $\alpha$ value to $0.01$.
...
First, we need to initialize four variables to keep track the summation of the squares of the gradients for the intercept and coefficient.
...
I know the variables are long, but it's better to be descriptive, and we will thank ourselves in the future.

Remember that the gradient of the cost function with respect to the intercept and coefficient are:
...
We would notice that the gradient of $\theta_0$ is just the difference between the prediction and the target value.
...
Remember that $G_{t,ii}$ is the sum of the squares of the gradients up to time $t$.
...
To update the intercept and coefficient values,
