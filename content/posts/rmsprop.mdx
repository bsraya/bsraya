...
Since there are two parameters, we need two new variables $g_{0,t}$ and $g_{1,t}$.
...
First, get a random number from the dataset, retreive a random data from the dataset,
and calculate the prediction. Remember, the gradient of the cost function
w.r.t. to the intercept is just the predicion error.
...
Second, determine the gradient of the cost function w.r.t. to the intercept and coefficient
with the prediction error term `error` assigned above.
...
Third, calculate the running average of the squared gradients $E[g^2]_t$.
