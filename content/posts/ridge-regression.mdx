...
Similar to what we did in previous posts, we need to estimate the best $\beta_0$ and $\beta_1$ using the Gradient Descent algorithm.
...
$$
\min _\beta\left( \frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2 + \lambda \sum_{j=1}^{p} |\beta_j^2| \right)
$$

where
1. $\frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2$ is Mean Squared Error.
2. $\lambda$ is the regularization parameter.
2. $\sum_{j=1}^{p} |\beta_j^2|$ is the L2 norm of the weights.
...
2. `serum_cholesterol` and `cholesterol_ratio` have a **moderate positive** correlation.
3. `serum_cholesterol` and `triglycerides` have a **moderate negative** correlation.
