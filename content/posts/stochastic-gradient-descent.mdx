...
Previously, we have learned that BGD updates parameters $\theta_0$ and $\theta_1$, the intercept and the coefficient values respectively, only after it has seen the entire dataset.
...
In other words, we want to find the best value for parameters $\theta_0$ and $\theta_1$ so that the line sits right in a position where it's close to most data points.
...
it updates parameters $\theta_0$ and $\theta_1$ after it has seen a random data point, as the name suggests.
...
$$
  J(\theta_i) = \frac{1}{N} \sum_{i=1}^N (f(x) - y_1)^2
$$

To be clear, we are to express $J(\theta_i)$ as $J(\theta_0, \theta_1)$ where $\theta_0$ is the intercept and $\theta_1$ is the coefficient so that
we know what parameters are being used.

However, since SGD updates parameters $\theta_0$ and $\theta_1$ after seeing a random data points, we do not have to divide the summation by $N$.

Thus, the cost function will be

$$
  J(\theta_0, \theta_1) = \sum_{i=1}^N (f(x) - y_i)^2
$$

The update rules stay the same

$$
  \theta_{i} = \theta_i - \alpha \cdot \frac{\partial}{\partial \theta_i} J(\theta_0, \theta_1)
$$

where $\theta_i$ is the $i$-th coefficient we want to update and $\alpha$ is the learning rate.

Applying Power Rule to the cost function, we have

$$
    \frac{\partial}{\partial \theta_i} J(\theta_0, \theta_1) = 2(f(x) - y_i) \frac{\partial}{\partial \theta_i} (f(x) - y_i)
$$

Let's partially derive the cost function with respect to parameters $\theta_0$ and $\theta_1$.

$$
    \begin{aligned}
        \frac{\partial}{\partial \theta_0} J(\theta_0, \theta_1)
        &= 2 (f(x) - y_i) \frac{\partial}{\partial \theta_0} (\theta_0 + \theta_1 x - y_i) \\
        &= 2 (f(x) - y_i)
    \end{aligned}
$$

$$
    \frac{\partial}{\partial \theta_1} J(\theta_0, \theta_1) = 2(f(x) - y_i)x
$$
...
$$
  J(\theta_0, \theta_1) = \frac{1}{2}(f(x) - y_i)^2
$$

Thus the partial derivatives with respect to $\theta_0$ and $\theta_1$ will be

$$
    \begin{aligned}
        \frac{\partial}{\partial \theta_0} J(\theta_0, \theta_1) = (f(x) - y_i) \\
        \frac{\partial}{\partial \theta_1} J(\theta_0, \theta_1) = (f(x) - y_i) x
    \end{aligned}
$$
...
$$
    \begin{aligned}
        \theta_0 = \theta_0 - \alpha \cdot (f(x) - y_i) \\
        \theta_1 = \theta_1 - \alpha \cdot (f(x) - y_i)x
    \end{aligned}
$$

The equations above is a variation of Gradient Descent algorithm that helps us to approximate the minimum value of the cost function by updating $\theta_0$ and $\theta_1$ iteratively.
...
def sgd(x, y, df, epochs, alpha = 0.01):
    ...
    for i in range(1,epochs):
        ...
        t0_error = prediction - y_i
        t1_error = (prediction - y_i) * x_i
