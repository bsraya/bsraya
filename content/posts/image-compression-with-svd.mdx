...
Note that all images consists of 3 layers, where each layer represents the red, green, and blue channel.
In order to prove that, let's separate the image into 3 layers.
...
I am sure you would be able to tell the difference between the original image and the reconstructed image. 
...
{/* Colored images are a bit more complicated than gray images since we have to deal with 3 layers of $768 \times 1024$ matrices. */}