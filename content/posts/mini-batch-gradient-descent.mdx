...
def mbgd(x, y, epochs, df, batch_size = 64, alpha = 0.01):
  ...
  for epoch in range(epochs):
    for x, y in zip(x_batches, y_batches):
      sum_error = 0.0
      predictions = predict(intercept, coefficient, x)
      t0_error = (1/batch_size) * np.sum(predictions - y)
      t1_error = (1/batch_size) * np.sum((predictions - y) * x)
...
![MSE values produced by MBGD with different batch sizes](/assets/posts/mini-batch-gradient-descent/mse.png "MSE values produced by MBGD with different batch sizes")
...
![TSS values produced by MBGD with different batch sizes](/assets/posts/mini-batch-gradient-descent/rss.png "TSS values produced by MBGD with different batch sizes")
...
![Regression Lines](/assets/posts/mini-batch-gradient-descent/regression-lines.png "Regression Lines")
...
![Regression Line Movements](/assets/posts/mini-batch-gradient-descent/mbgd.gif "Regression Line Movements")
...
![The intercept and the coefficient variable on a contour map](/assets/posts/mini-batch-gradient-descent/contour.png "The intercept and the coefficient variable on a contour map")
