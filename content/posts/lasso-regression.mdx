...
description: How to create Lasso Regression and how it can handle multicollinearity
date: 2024-02-05
...
we need to estimate the best $\beta_0$ and $\beta_1$ using the Gradient Descent algorithm.
What the Gradient Descent algorithm does is to update the $\beta_0$ and $\beta_1$ values based on the cost function and the learning rate.
...
$$
\begin{gather*}
\beta_0 = \beta_0 - \alpha \frac{\partial}{\partial \beta_0} J(\beta_0, \beta_1, \dots, \beta_p) \\
\beta_1 = \beta_1 - \alpha \frac{\partial}{\partial \beta_1} J(\beta_0, \beta_1, \dots, \beta_p)x_1 \\
\cdots \\
\beta_p = \beta_n - \alpha \frac{\partial}{\partial \beta_n} J(\beta_0, \beta_1, \dots, \beta_p)x_p \\
\end{gather*}
$$

where $\alpha$ is the learning rate, $\beta_p$ is the $p$-th parameter, $J$ is the cost function, and $x_p$ is the $p$-th feature.

Since we only have $\beta_0$ and $\beta_1$, we can simplify the equation above to:

$$
\begin{gather*}
\beta_0 = \beta_0 - \alpha \frac{\partial}{\partial \beta_0} J(\beta_0, \beta_1) \\
\beta_1 = \beta_1 - \alpha \frac{\partial}{\partial \beta_1} J(\beta_0, \beta_1)x_1 \\
\end{gather*}
$$
...
$$
\min _\beta\left( \frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2 + \lambda \sum_{j=1}^{p} |\beta_j| \right)
$$

where
1. $\frac{1}{N} \sum_{i=1}^{n} (\hat{y}_i - y_i)^2$ is Mean Squared Error.
2. $\lambda$ is the regularization parameter.
2. $\sum_{j=1}^{p} |\beta_j|$ is the L1 norm of the weights.
...
By looking at the result of these two methods, we can see that `MedInc`, `AveRooms`, `AveBedrms`, `Latitude`, and `Longitude` suffer from multicollinearity.

## Implementation

In this post, we are going to use the Californian Housing Dataset from the Scikit-Learn library and split the dataset into training and testing sets.
...
After loading the data, we are going to split the dataset into training and testing sets.
Once split, the data can be standardized using `StandardScaler` from the Scikit-Learn.
...
2. Lasso Regression is very effective at handling data that suffers from severe multicollinearity.
3. Lasso Regression can be used to select important features in the dataset.
4. Lasso Regression can be used to reduce the complexity of the model.
