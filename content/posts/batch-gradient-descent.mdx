...
We have also acquired two equations that are reponsible for updating $\theta_0$ and $\theta_1$. Namely,

$$
  \begin{aligned}
    \theta_0 = \theta_0 - \frac{\alpha}{N} \sum_{i=1}^N (f(x) - y_i) \\
    \theta_1 = \theta_1 - \frac{\alpha}{N} \sum_{i=1}^N (f(x) - y_i) x
  \end{aligned}
$$
...
def bgd(x, y, epochs, df, alpha = 0.01):
    ...
    for epoch in range(1, epochs):
        predictions = predict(intercept, coefficient, x)
        t0_error = (1/len(x)) * np.sum(predictions - y)
        t1_error = (1/len(x)) * np.sum((predictions - y) * x)
...
![The movement of the intercept and the coefficient variabels on a contour map](/assets/posts/batch-gradient-descent/contour.png "The movement of the intercept and the coefficient variabels on a contour map")
