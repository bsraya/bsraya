...
<Alert status="info" variant="left-accent">
  <AlertIcon />
  Most of the code in this post is from my previous post, [Gradient Descent series](/blog/gradient-descent).
</Alert>
...
I personally prefer typical `if-else` way of coding since it makes the code more readable and easy to understand.
With list comprehension, even though it makes the code shorter, but it's rather hard for most people to understand.
However, we are going to test if it's faster than list append + `if-else` way.
...
Why is `np.array([])` slower at appending compared to list? If you print out `np.append.__doc__`, you will see the following docstring.
...
The reason why `np.append` is slower than `list.append` because it always make a copy of a thing and store it at a new array. That particular process is resource expensive.
...
1. We have tested list and numpy array
2. There are two ways working with list: list with if-else or list comprehension
3. Appending elements to numpy array is the slowest since it always make a copy of a thing and store it at a new array
4. List comprehension is the fastest compared to list with if-else and numpy array
