...
export default function SeriesTable({ title, series }: { title: string, series: SeriesPost[] }) {
  return (
    <div className="mb-32 p-5 rounded-lg border not-prose w-2/3 text-gray-800">
      <p className="text-2xl mb-5 underline">{title}: {series.length} Parts</p>
      <ol className="list-decimal list-inside flex flex-col gap-3 ml-3">
        {
          series.map(({ title, current }: SeriesPost) => {
            const slug = title.toLowerCase().replace(/ /g, '-');
            return (
              <li key={slug} className={`hover:font-bold ${current ? 'font-bold' : ''}`}>
